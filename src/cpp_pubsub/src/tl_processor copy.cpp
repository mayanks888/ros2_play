...
for(auto m : msg->detections)
{
  ...
  for (size_t col = 0; col < detected_bboxes.size(); ++col) {
    // std::cout<<col;
    std::cout<<detected_bboxes[col].debug_roi[col]<<std::endl;
...
void Processor::Select(std::vector<Image_Light> &hdmap_bboxes,
  ...
  auto max_area_refined_bbox =
      std::max_element(refined_bboxes.begin(), refined_bboxes.end(),[](const Image_Light lhs, const Image_Light rhs) 
      {
                         return lhs.rectified_roi.area() < rhs.rectified_roi.area();
                       });
  ...
  for (size_t row = 0; row < hdmap_bboxes.size(); ++row) {
    ...
    for (size_t col = 0; col < refined_bboxes.size(); ++col) {
      ...
      //     GetCenter(refined_bboxes[col].rectified_roi);
      // auto width_refine = refined_bboxes[col].rectified_roi.width;
      cv::Point2f center_refine = GetCenter(refined_bboxes[col].debug_roi[col]);
      auto width_refine = refined_bboxes[col].debug_roi[col].width;
      ...
      // larger area => 1
      double area_score = 1.0 * refined_bboxes[col].debug_roi[col].area() / (*max_area_refined_bbox).debug_roi[0].area();

      // whden numerator=1， denominator is very small,
      // converting to int might reduce to same value, here uses 1000
      // + 0.05 to prevent score = 0
      // score * weight h
      score_matrix[row][col] = (0.05 + 0.4 * refined_bboxes[col].debug_roi_detect_scores[col] + 0.2 * distance_score + 0.2 * width_score + 0.2* area_score );
    std::cout<<score_matrix[row][col]<<std::endl;
    std::cout<<area_score<<std::endl;
    }
  }
